...
-   **Owner Analytics** (Admin-Only):
    -   Comprehensive business intelligence dashboard for TRACER C2FS owners
    -   Real-time metrics: total revenue, agent net (commissions), merchant counts, active accounts
    -   Visualizations: revenue trends, market share by processor, processor performance comparison
    -   Top performers: top 10 branches and merchants by revenue
    -   Commission analysis by processor with contribution percentages
    -   **Secure Authentication**: Session-based admin authentication with httpOnly cookies, session regeneration on login, and session destruction on logout
    -   **Security Features**: Protection against XSS attacks (httpOnly cookies), session fixation (ID regeneration), and cookie replay (explicit clearing)
-   **Dashboard Views**:
